...
Normality assumptions arise frequently in statistics and econometrics. Hypothesis testing relies on a normally distributed sampling distribution to allow us to know the likelihood of a particular test statistic occuring (aka the basis of [statistical significance](https://en.wikipedia.org/wiki/Statistical_significance)). General linear models require normally distributed residuals. Etc etc etc.
...
We'll begin by taking `t` trials of size `n` and storing them in a matrix, then calculating the mean of each trial in a function as such:
...
After using `set.seed()` to ensure reproducibility, we define `t` and `n` with some "low" values. The rbinom function will return random draws of 1 according to our defined probabililty, which we store in `samps`. Finally, we use our `sample.means` function above to calculate the average of each trial, giving us a total of `t` sample means.
```{r, warning = FALSE, message = FALSE}
set.seed(1000)
...
With increased sample, one can see the smooth bell shaped curve emerging on the right-hand side. Hovering over the center of this distribution will show its center around our defined mean of .8, thanks to the magic of the central limit theorem.
